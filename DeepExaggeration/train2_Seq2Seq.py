...
import model2_2_targetlatent as sketch_rnn_model
import utils
...
# tf.flags 定义全局参数设置
tf.app.flags.DEFINE_string('data_dir', '../inputData/','the dataset filepath')
tf.app.flags.DEFINE_string('log_root', 'parameters','Directory to store model')
...
def train(sess, model, eval_model, source_set, target_set):
    ...
    for _ in range(hps.num_steps):
        ...
        idx = np.random.permutation(range(0, 472))[0:64] 
        ...
        if step % hps.save_every == 0 and step > 0:
            (test_cost, test_r_cost,) = evaluate_model(sess, eval_model, source_set, target_set)
