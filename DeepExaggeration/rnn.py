...
class LSTMCell(tf.contrib.rnn.RNNCell):
    ...
    def get_output(self, state):
        unused_c, h = tf.split(state, 2, 1) #输出有两个状态值，分别是cell值和h（整个输出）
        return h

    def __call__(self, x, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            c, h = tf.split(state, 2, 1)
            x_size = x.get_shape().as_list()[1] # 输入大小

            w_init = None  # uniform
            h_init = lstm_ortho_initializer(1.0) 
            #每一个神经元的输入为外部输入 x 和上一时刻的输出 h
            ...
            concat = tf.concat([x, h], 1) #输入
            w_full = tf.concat([w_xh, w_hh], 0) # 权值
            hidden = tf.matmul(concat, w_full) + bias #隐层值 = w * x + b

            i, j, f, o = tf.split(hidden, 4, 1) #四个门的参数由隐层值分为4份 
            #i -- input gate 参数 wx + wh
            #j--  输入 wx + wh
            #f -- forget gate 参数 wx + wh
            #o -- output gate 参数 wx + wh

            if self.use_recurrent_dropout:
                g = tf.nn.dropout(tf.tanh(j), self.dropout_keep_prob)
            else:
                g = tf.tanh(j) # tanh(wx + wh)

            new_c = c * tf.sigmoid(f + self.forget_bias) + tf.sigmoid(i) * g 
            #新的cell状态 =  sigmoid(f) * c(t-1) + sigmoid(i) * g
            new_h = tf.tanh(new_c) * tf.sigmoid(o) #新的h值  

            return new_h, tf.concat([new_c, new_h], 1)  # fuk tuples.


def layer_norm_all(h, batch_size, base, num_units, scope='layer_norm', reuse=False,
                   gamma_start=1.0, epsilon=1e-3, use_bias=True):
    ...
    with tf.variable_scope(scope):
        ...
        gamma = tf.get_variable('ln_gamma', [4 * num_units],
...
def layer_norm(x,num_units,scope='layer_norm',reuse=False,
          gamma_start=1.0,epsilon=1e-3, use_bias=True):
    axes = [1]
    ...
    with tf.variable_scope(scope):
        if reuse is True:
            tf.get_variable_scope().reuse_variables()
        gamma = tf.get_variable('ln_gamma', [num_units],
                        initializer=tf.constant_initializer(gamma_start))
        if use_bias:
...
class HyperLSTMCell(tf.contrib.rnn.RNNCell):
    def __init__(self,num_units,forget_bias=1.0,use_recurrent_dropout=False,
             dropout_keep_prob=0.90,use_layer_norm=True,hyper_num_units=256,
             hyper_embedding_size=32,hyper_use_recurrent_dropout=False):
        self.num_units = num_units
        self.forget_bias = forget_bias
        self.use_recurrent_dropout = use_recurrent_dropout
        self.dropout_keep_prob = dropout_keep_prob
        ...
        with tf.variable_scope(scope):
            ...
            if use_bias:
                zb = super_linear(self.hyper_output,
                    embedding_size,init_w='gaussian',weight_start=0.01,
                    use_bias=False,bias_start=0.0,scope='zb')
    ...
    def __call__(self, x, state, timestep=0, scope=None): 
        with tf.variable_scope(scope or type(self).__name__):
            ...
            self.hyper_state = tf.concat([total_h[:, self.num_units:], 
                               total_c[:, self.num_units:]], 1)
