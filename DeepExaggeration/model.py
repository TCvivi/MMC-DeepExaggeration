...
def get_default_hparams():
    ...
    hparams = tf.contrib.training.HParams(
        source = 'sourcesOffset12.npy',  
        target = 'targetsOffset12.npy', 
        sourcesTest = 'sourceTest12.npy',  
        targetsTest = 'targetTest12.npy',
...
class Model(object):
    ...
    def build_model(self, hps): 
        ...
        self.source_input = tf.placeholder(dtype=tf.float32,
                       shape=[self.hps.batch_size, self.hps.max_seq_len + 1, 12])
        ...
        if hps.conditional:  
            ...
            #输入数据，得到隐含变量h            
            actual_input_x = tf.concat([self.decoder_input_source_x,
                               self.decoder_input_target_x], 2) 
        ...
        n_out = (10 + self.num_mixture * 6) 
        #解码器输出y的维度为 5M + M + 9，分别表示高斯函数参数、权重、（p1,p2,...,p9）
        ...
        def get_lossfunc(z_pi, z_mu1, z_mu2, z_sigma1, z_sigma2, z_corr,
            ...
            fs = 1.0 - pen_data[:,9]  #如果最后一个点为1，则为终点概率为0，反之，为1. 
        ...
        def get_mixture_coef(output):
            ...
            z = output
            z_pen_logits = z[:, 0:10]  # pen states z的前三个值为（p1,p2,p3）
            #剩下的6个分别为权重pi，miu(x),miu(y), z_sigma1, z_sigma2, z_corr
            z_pi, z_mu1, z_mu2, z_sigma1, z_sigma2, z_corr = tf.split(z[:, 10:], 6, 1)
        ...
        target = tf.reshape(self.output_x, [-1, 12]) #目标输出（x,y ,p1,p2,p3）
        [x1_data, x2_data, p1, p2, p3, p4, p5, p6, p7, p8, p9, p10] = tf.split(target, 12, 1)
        pen_data = tf.concat([p1, p2, p3, p4, p5, p6, p7, p8, p9, p10], 1) 
...
def sample(sess, model, seq_len=68, temperature=1.0, greedy_mode=False, z=None):
    """Samples a sequence from a pre-trained model."""

    tf.logging.info('model- sample 根据模型对真实图片进行操作')
    ...
    #从原始图片中随机取一张图片            
    result = np.zeros((69, 12), dtype=float)
    peoples = np.load('../data/sources.npy')
    sample_idx = random.randint(0,300)
    tf.logging.info('sample的第 %i 张图片',sample_idx)
    randpeople = peoples[sample_idx] 
    result[0:68, :] = randpeople[:, :]
    result[1:, :] = result[:-1, :]
    result[0, :] = 0.0
    encoder_input_x = result[1:,:] 
    decoder_input_source_x = result[0:68,:] 
    prev_x = np.zeros((1, 1, 12), dtype=np.float32)
    prev_x[0, 0, 2] = 1  # initially, we want to see beginning of new stroke
    
    prev_state = sess.run(model.initial_state, feed_dict={model.encoder_input_x: encoder_input_x})

    cartoon = np.zeros((seq_len, 12), dtype=np.float32)
    mixture_params = []
    greedy = False
    temp = 1.0
                          
    for i in range(seq_len):
        feed = {model.encoder_input_x: encoder_input_x[i],
              model.decoder_input_source_x: decoder_input_source_x[i],
              model.decoder_input_target_x: prev_x,
        ...
        cartoon[i, :] = [next_x1, next_x2, eos[0], eos[1], eos[2], eos[3], eos[4],
                   eos[5], eos[6], eos[7], eos[8], eos[9]]
        ...
        prev_x = np.zeros((1, 1, 12), dtype=np.float32)
        prev_x[0][0] = np.array(
            [next_x1, next_x2, eos[0], eos[1], eos[2], eos[3], eos[4],
                   eos[5], eos[6], eos[7]], eos[8], eos[9], dtype=np.float32)
        prev_state = next_state

    return cartoon, mixture_params
