...
# tf.flags 定义全局参数设置
tf.app.flags.DEFINE_string('data_dir', '../data/','the dataset filepath')
tf.app.flags.DEFINE_string('log_root', '/default','Directory to store model')
...
def evaluate_model(sess, model, data_set):
    ...
    for batch in range(data_set.num_batches):
        unused_orig_x, unused_orig_y, x, y, s = source_set.get_batch(batch)
        feed = {model.input_data: x, model.decoder_input: y, model.sequence_lengths: s}
        (cost, r_cost) = sess.run([model.cost, model.r_cost], feed)
        total_cost += cost
        total_r_cost += r_cost

    total_cost /= (source_set.num_batches)
    total_r_cost /= (source_set.num_batches)
...
def train(sess, model, eval_model, source_set, target_set):
    ...
    # 打印网络参数
    tf.logging.info('train-171- Total trainable variables %i.', count_t_vars) #1901824
   ...
   # valid_cost = 0.0
    ...
    for _ in range(hps.num_steps):
        tf.logging.info('train- 开始迭代运行啦======================================')
        step = sess.run(model.global_step)
        curr_learning_rate = ((hps.learning_rate - hps.min_learning_rate) *
                          (hps.decay_rate)**step + hps.min_learning_rate)

        tf.logging.info('train- 按batch取出source和target数据')
        idx = np.random.permutation(range(0, 364))[0:20] #数据集数目、batch大小改了要来改这里！！！！
        _, x, s1 = source_set._get_batch_from_indices(idx)
        _, y, s2 = target_set._get_batch_from_indices(idx)
 
        tf.logging.info('train- 将数据流入模型图中')
        feed = {
        ...
        if step % 10 == 0 and step > 0: #每10个step打印一次结果
            tf.logging.info('train- 每10个epoch打印一次结果=========================')
            end = time.time()
            ...
            output_values = (step, curr_learning_rate, train_cost, r_cost, time_taken)
