...
import model5_2_R_C as sketch_rnn_model
import utils
...
tf.app.flags.DEFINE_string('log_root', 'parameters_5_2_bs128','Directory to store model')
...
def evaluate_model(sess, model, sources, targets):
    ...
    for batch in range(sources.num_batches): 
        start_idx = batch * 128
        indices = range(start_idx, start_idx + 128)
        ...
        (cost, r_cost, il_cost) = sess.run([model.cost, model.r_cost, model.il_cost], feed)
        total_cost += cost
        total_r_cost += r_cost
        total_il_cost += il_cost

    total_cost /= (sources.num_batches)
    total_r_cost /= (sources.num_batches)
    total_il_cost /= (sources.num_batches)
    return (total_cost, total_r_cost, total_il_cost)
...
def train(sess, model, eval_model, source_set, target_set):
    ...
    for _ in range(hps.num_steps):
        ...
        idx = np.random.permutation(range(0, 475))[0:128] 
        ...
        (train_cost, r_cost, il_cost, _, train_step, _) = sess.run([
            model.cost, model.r_cost, model.il_cost, model.final_state,
        ...
        if step % 10 == 0 and step > 0: #每10个step打印一次结果
            ...
            il_summ = tf.summary.Summary()
            il_summ.value.add(tag='Train_identify_Cost', simple_value=float(il_cost)) #KL散度
            ...
            output_format = ('step: %d, lr: %.6f, ilw: %0.4f, cost: %.4f, '
                       'recon: %.4f, il: %.4f, train_time_taken: %.4f')
            output_values = (step, curr_learning_rate, curr_il_weight, 
                             train_cost, r_cost, il_cost, time_taken)
            ...
            summary_writer.add_summary(il_summ, train_step)
        ...
        if step % hps.save_every == 0 and step > 0:
            (test_cost, test_r_cost, test_il_cost) = evaluate_model(
            ...
            test_il_summ = tf.summary.Summary()
            test_il_summ.value.add(tag='test_identify_Cost', simple_value=float(test_il_cost))
